...
class ChoiceFrame(tk.LabelFrame):
    def __init__(self, master, **kw):
        ...
        self.music_listbox = tk.Listbox(
            self, width=42, height=20, selectmode=tk.EXTENDED
        )
        self.mlb_y_scrollbar = tk.Scrollbar(self, command=self.music_listbox.yview)
        self.mlb_x_scrollbar = tk.Scrollbar(
            self, orient=tk.HORIZONTAL, command=self.music_listbox.xview
        )
    ...
    def config_gui(self):
        self.music_listbox.config(
            xscrollcommand=self.mlb_x_scrollbar.set,
...
class DecorationFrame(tk.LabelFrame):
    def __init__(self, master, **kw):
        ...
        self.label = tk.Label(
            self, text='Please select a music.', font=('华文行楷', 10), fg='blue'
        )
...
class ShowMusicFrame(tk.LabelFrame):
    def __init__(self, master, **kw):
        ...
        self.llb_x_scrollbar = tk.Scrollbar(
            self, orient=tk.HORIZONTAL, command=self.lyric_listbox.xview
        )
    ...
    def config_gui(self):
        # 与Listbox联动
        self.lyric_listbox.config(
            xscrollcommand=self.llb_x_scrollbar.set,
...
class ButtonFrame(tk.LabelFrame):
    def __init__(self, master, **kw):
        ...
        self.play_mode_box = ttk.Combobox(
            self, width=7, state=tk.DISABLED  # 'readonly'
        )
